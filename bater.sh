...
function heat_on(){
	...
	then
		...
		echo -e "$(printf '%(%d-%m %H:%M:%S)T\n') C state:${h_state}\tmode:${t_mode}\tsp:${t_sp}\tair:${t_air}\tfloor:${t_floor}\ttoh:${toh}" >> ~/scripts/bater/heating.log
	fi
}
...
echo -e "$(printf '%(%d-%m %H:%M:%S)T\n') I state:${h_state}\tmode:${t_mode}\tsp:${t_sp}\tair:${t_air}\tfloor:${t_floor}\ttoh:${toh}" >> ~/scripts/bater/heating.log
